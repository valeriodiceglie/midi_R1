...
from src.midiR1.utils.metrics import plot_metrics
...
def validate(model, data_loader, criterion, pad_token_id, device):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Validation", leave=False):
            input_ids, attention_mask, target_ids = batch["input_ids"], batch["attention_mask"], batch["labels"]
    ...
    for epoch in range(config['num_epochs']):
        ...
        for batch_idx, batch in progress_bar:
            ...
            if (batch_idx + 1) % gradient_accumulation_steps == 0:
                ...
                if not isinstance(scheduler, CosineAnnealingLR) and not isinstance(scheduler,
